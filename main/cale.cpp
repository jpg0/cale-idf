...
static int g_http_max_age_seconds = -1; // Global static for HTTP Cache-Control max-age
...
esp_err_t _http_event_handler(esp_http_client_event_t *evt)
{
        ...
        if (strcmp(evt->header_key, "Cache-Control") == 0) {
            ...
            if (max_age_str != NULL) {
                ...
                if (parsed_max_age > 0) { // atoi returns 0 on error, ensure it's a valid positive number
                    g_http_max_age_seconds = parsed_max_age;
                    ESP_LOGI(TAG, "max-age found: %d", g_http_max_age_seconds);
                } else {
    ...
    case HTTP_EVENT_ON_FINISH:
        ...
        deepsleep(g_http_max_age_seconds);
        break;
...
{
    g_http_max_age_seconds = -1; // Reset before each new HTTP request cycle
    /**
